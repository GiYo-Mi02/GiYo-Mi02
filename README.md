
<h2 align="center">✨ Gio | Full Stack Dev • CS Student • Builder ✨</h2>

<p align="center">
  <i>"Design with intention. Build with purpose."</i>
</p>

---

### 🌿 About Me  
Hi! I'm **Gio**, a Computer Science student passionate about  
**Full Stack development, clean design, and meaningful user experiences.**  
I love building tools that help people — whether it's an automation script  
or a cultural heritage kiosk built from scratch.

---

### 🛠 Tech Stack  
<p align="center">
  <img src="https://skillicons.dev/icons?i=html,css,js,php,py,java,react,nextjs,typescript,git,figma,supabase" />
</p>

---

### 🚀 Current Projects  
- ⚙️ Automation tools for data entry + databases  
- 🧪 Making a working system for the University

---

### 📚 Currently Learning  
- Deepening my knowledge in **Python, Java, and JavaScript**  
- **Database systems** (Supabase / PostgreSQL)  
- **UI/UX** → wireframing & interface flow  
- **DSA** for cleaner and smarter solutions  

---

### 🌐 Connect With Me  
<p align="left">
<a href="#"><img src="https://skillicons.dev/icons?i=github" height="40"/></a>
<a href="#"><img src="https://skillicons.dev/icons?i=linkedin" height="40"/></a>
<a href="#"><img src="https://skillicons.dev/icons?i=gmail" height="40"/></a>
</p>

📫 **Email:** ggonzales.k12254495@umak.edu.ph 
🪪 **Portfolio:** /*Currently Work in Progress*/

---

<p align="center">
  <i>Thanks for visiting! Feel free to explore my projects ✨</i>
</p>
